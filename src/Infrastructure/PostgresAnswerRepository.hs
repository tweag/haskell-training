...
postgresAnswerRepository connection = AnswerRepository
  { allForSubmission = postgresAllAnswersForSet connection
  , allForQuestion = postgresAllAnswersForQuestion connection
  }

postgresAllAnswersForSet
  :: Connection
  -> Id Domain.Submission
  -> ExceptT QueryError IO [Identified Domain.Answer]
postgresAllAnswersForSet connection answerSetId = do
  answers <- ExceptT $ run
    (statement () . select $ DB.submissionAnswers answerSetId)
    connection
  pure $ deserializeAnswer <$> answers

postgresAllAnswersForQuestion
  :: Connection
  -> Id Domain.Question
  -> ExceptT QueryError IO [Identified Domain.Answer]
postgresAllAnswersForQuestion connection questionId = do
  answers <- ExceptT $ run
    (statement () . select $ DB.questionAnswers questionId)
    connection
  pure $ deserializeAnswer <$> answers
